node {
      ...
      stage('docker build') {
        ...
        sh 'docker build --build-arg ENVIRONMENT=dev -t  mungta-notice-service-dev .'
        sh 'docker tag mungta-notice-service-dev mungtaregistry.azurecr.io/mungta/dev/notice'
      }
      stage('deploy') {
        withCredentials([azureServicePrincipal('azure_service_principal')]) {
          ...
          echo '---------AKS 배포------------'
          sh 'docker push mungtaregistry.azurecr.io/mungta/dev/notice'
          sh 'kubectl apply -f notice-service-deploy-dev.yml'
          sh 'kubectl rollout restart deployment notice --namespace=mungta'
